...
@Component
public class S3ThreatScanWorker implements Worker {

    @Override
    public String getTaskDefName() {
        return "s3_threat_scan";
    }
    ...
    public TaskResult execute(Task task) {
        ...
        try {
            ...
            Boolean status = Util.threatScan(fileLocation);
            result.setStatus(TaskResult.Status.COMPLETED);
            result.addOutputData("fileLocation", url);
            result.addOutputData("isThreatFound", true);
